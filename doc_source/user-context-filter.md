...
You can filter a query response to remove documents from a user's search results based on their user name and group membership\. You can filter search results by user token, user ID, or user attribute\. Amazon Kendra can map users to their groups\.
...
When you query the documents, you want to ensure that the correct ACL is applied\. For example, you can use an OpenID token\. It uses a JSON format\. When you issue a query, Amazon Kendra extracts and validates the token\. Amazon Kendra pulls the user and group information and runs the query\. All of the documents the user has access to including public documents are returned\.
...
The Amazon FSx group and user names are mapped as follows:
+ `_group_ids` – Group names are present on files where there are set access permissions\. They are mapped from the name of the group in the directory service of Amazon FSx\.
+ `_user_id` – User names are present on files where there are set access permissions\. They are mapped from the user name in the directory service of Amazon FSx\.
...
The Slack user emails as the IDs are mapped as follows:
+ `_user_id` – User names are present on Slack messages and channels where there are set access permissions\. They are mapped from the user emails as the IDs in Slack\.
...
+ `_group_ids` – Group names are present on files where there are set access permissions\. They are mapped from the name of the group in Box\.
+ `_user_id` – User names are present on files where there are set access permissions\. They are mapped from the user emails as the user IDs in Box\.
...
The Quip user emails as the IDs are mapped as follows:
+ `_user_id` – User names are present on Quip files where there are set access permissions\. They are mapped from the user emails as the IDs in Quip\.
...
The Jira IDs are mapped as follows:
+ `_user_id` – User names are present on files where there are set access permissions\. They are mapped from the user emails as the user IDs in Jira\.
...
The GitHub user emails as the IDs are mapped as follows:
+ `_user_id` – User names are present on the files where there are set access permissions\. They are mapped from the user emails as the IDs in GitHub\.
