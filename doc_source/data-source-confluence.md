...
You can use Amazon Kendra to index blogs, pages, and attachments to either blogs and pages that are hosted on an Atlassian Confluence cloud or Atlassian Confluence server instance\. For a walk\-through of how to use Confluence in the console, see [Getting started with a Confluence data source \(console\)](https://docs.aws.amazon.com/kendra/latest/dg/getting-started-confluence.html)\.

By default, Amazon Kendra indexes regular spaces, you can configure Amazon Kendra to index personal and archived spaces\. To exclude or include specific blogs, pages, and spaces from indexing, you use a list\.
