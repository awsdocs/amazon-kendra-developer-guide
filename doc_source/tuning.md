...
You can specify date, number, or string attributes to tune an index or tune at the query level\. You can also tune fields or attributes that are of the type `StringList`\. Each field or attribute has specific criteria for when it boosts a result\.
