...
If your database isn't running on an Amazon VPC, you can connect your database to your Amazon VPC using a Virtual Private Network \(VPN\)\. You get a default VPC when you create your Amazon account\. For information on setting up a VPN, see the [ AWS Virtual Private Network Documentation](https://docs.aws.amazon.com/vpn/)\. 
...
1. From the left menu, choose **Subnets**\.

1. From the subnet list, choose the subnet that contains your database server\.

1. In the description tab, the identifier is in the **Subnet ID** field\.

**To view security group identifiers**

1. Sign in to the AWS Management Console and open the Amazon VPC console at [https://console\.aws\.amazon\.com/vpc/](https://console.aws.amazon.com/vpc/)\.

1. From the left menu, choose **Security Groups**\.

1. From the security group list, choose the group that you want the identifier for\.

1. In the description tab, the identifier is in the **Group ID** field\.
...
1. From the left menu, choose **Route tables** then choose **Create route table**\.
...
1. From the left menu, choose **NAT Gateways** then choose **Create NAT Gateway**\.

1. In the **Subnet** field, choose the subnet that should be the public subnet and note the subnet ID\.

1. If you don't have an available Elastic IP, choose **Create New EIP** and then choose **Create a NAT Gateway** and then choose **Close**\.

1. From the left menu, choose **Route Tables**\.

1. From the route table list, choose **Private subnet route table** that you created in step 3\. From the **Actions** menu, choose **Edit Routes**\. 

1. Choose **Add route**\. Add the destination 0\.0\.0\.0/0 to allow all outgoing traffic to the Internet\. In the **Target**, choose **NAT Gateway** and then choose the gateway that you created in step 4\. Finally, choose **Save routes** and then choose **Close**\.

1. From **Actions**, choose **Edit subnet associations\.**

1. Select the subnets that you want to be private\. Do not select the subnet with the NAT gateway that you noted above\.
...
1. From the left menu, choose **Security Groups** and then choose **Create security group**\.

1. In **Security group name** type **MySQLSecurityGroup**\. Provide a description, then choose your VPC from the drop down list\. Choose **Create** and then choose **Close**\.

1. Choose the **Inbound** tab\.

1. Choose **Edit rules** and then choose **Add Rule**

1. For a MySQL database, type 3306 for the **Port Range**\. For the **Source**, type the CIDR of your VPC\. Choose **Save rules** and then choose **Close**\.
...
1. From the left menu, choose **Subnet groups** and then choose **Create DB Subnet Group**\.
...
1. From the left menu, choose **Databases** and then choose **Create database\.**
...
1. From the left menu, choose **Indexes** and then choose your index\.
